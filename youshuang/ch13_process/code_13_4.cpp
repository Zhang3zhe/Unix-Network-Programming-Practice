...
int run_child(int idx, client_data* users, char* share_mem){
	...
	while(!stop_child){
		...
		for(int i = 0; i < number; ++i){
			...
			if(sockfd == connfd && (events[i].events & EPOLLIN)){ // 本子进程负责的客户连接有数据到达
			...
			} else if(sockfd == pipefd && (events[i].events & EPOLLIN)){ // 主进程通知本进程将数据发送到对应的客户端
				int client  = 0;
				// 接收主进程的通知
		...
		for(int i = 0; i < number; ++i){
			...
			if(sockfd == listenfd){		//新的客户连接到来
				...
				if(ret == -1 || ret == 0){
					...
					// 向负责处理第child个客户连接之外的其他进程发送消息，通知他们有客户数据要写
