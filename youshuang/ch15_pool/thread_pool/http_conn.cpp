...
void http_conn::init(int sockfd, const sockaddr_in & addr){
	...
	// 如下两行为了避免TIME_WAIT状态，仅用于debug，实际使用应去掉
	int reuse = 1;
...
http_conn::HTTP_CODE http_conn::parse_request_line(char* text){
	...
	*m_version++ = '\0';
	...
	// 仅支持HTTP/1.1
	if(strcasecmp(m_version, "HTTP/1.1") != 0){	// 后面本来有回车换行符，但已经事先被行解析函数填充为'\0'
		return BAD_REQUEST;
	}
...
// 当得到一个完整的、正确的HTTP请求时，分析目标文件的属性
// 如果目标文件存在，对所有用户可读，且不是目录，
// 则使用mmap将其映射到内存地址m_file_address处，并通知调用者文件获取成功
...
bool http_conn::write(){
	...
	while(1){
		...
		if(temp <= -1){
			...
			// 虽然在此期间服务器无法立即接受到同一客户的下一个请求，但这可以保证连接的完整性
			if(errno == EAGAIN){
