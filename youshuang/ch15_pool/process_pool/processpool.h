...
class process{
 ...
 public:
	pid_t m_pid;
	int m_pipefd[2];	// 用于父子进程之间通信的管道
};

template <typename T>
class processpool{
 private:
	// 构造函数设为私有，只能通过后面的create静态函数来创建processpool实例
	processpool(int listenfd, int process_number = 8);
	
 public:
	// 单例模式，以保证最多创建一个processpool实例
 ...
 private:
	...
	int m_epollfd;								// 每个子进程都有一个epoll内核事件表
	int m_listenfd;								// 监听socket
	int m_stop;									// 决定子进程是否停止运行
...
// 进程池构造函数，参数listenfd必须在创建进程池之前被创建
template <typename T>
...
void processpool<T>::run_child(){
	...
	while(!m_stop){
		...
		for(int i = 0; i < number; ++i){
			...
			if(sockfd == pipefd && (events[i].events & EPOLLIN)){
				int client = 0;
				// 从父子进程之间的管道读取数据，若读取成功，表示有新客户连接到达
			...
			} else if(events[i].events & EPOLLIN){	// 其他可读数据，必然是客户请求到来，调用逻辑处理对象的process方法处理之
	...
	users = NULL;
	close(pipefd);
	//close(m_listenfd);	// listenfd应该由创建者进行关闭
	close(m_epollfd);
}
...
void processpool<T>::run_parent(){
	...
	while(!m_stop){
		...
		for(int i = 0; i < number; ++i){
			int sockfd = events[i].data.fd;
			if(sockfd == m_listenfd){		//新的客户连接到来，采用Round Robin方式分配给一个子进程进行处理
	...
	//close(listenfd);	// 同样，listenfd应该由创建者进行关闭
	close(m_epollfd);
}
