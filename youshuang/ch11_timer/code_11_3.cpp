...
void timer_handler(){
	timer_lst.tick();
	// 因为alarm调用只会引起一次SIGALRM信号
	// 所以需要重新定时，以不断触发SIGALRM信号
	alarm(TIMESLOT);
}

// 定时器回调函数，它删除非活动链接socket上的注册事件，并关闭之
	...
	while(!stop_server){
		...
		for(int i = 0; i < number; ++i){
			...
			if(sockfd == listenfd){		// 处理新到的客户连接
				...
				// 创建定时器，设置其回调函数与超时时间，然后绑定用户数据
				// 最后将定时器加到链表timer_lst中
				...
				if(ret < 0){
					...
					if(errno != EAGAIN){
						...
						if(timer != NULL)	timer_lst.del_timer(timer);
					}
				} else if(ret == 0){
					// 如果对方已关闭连接，我们也关闭连接并移除定时器
					cb_func(&users[sockfd]);
					if(timer != NULL)	timer_lst.del_timer(timer);
				} else {
					// 如果某个客户连接上的数据可读，则要调整定时器，延迟关闭时间
					if(timer != NULL){
						time_t cur = time(NULL);
						timer->expire = cur + 3 * TIMESLOT;
